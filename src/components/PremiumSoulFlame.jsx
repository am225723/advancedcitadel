  ...
  const flameShader = useMemo(() => ({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // === Color Gradients ===
        // Layered mixing based on distance from center and height
        vec3 finalColor = uCoreColor * coredensity * 1.8; // Boost core contribution
        ...
        finalColor += midColorBlended * middensity * 1.3;

        // Outer-color blending: fiery orange with subtle cool wisps at top/edges
        vec3 edgeColorBlended = mix(uEdgeColor, uOuterWispColor, heightGradientSmooth * 0.8 * fresnel); // Wisp color influenced by height and fresnel
        finalColor += edgeColorBlended * outerdensity * 0.7;

        // Add subtle detail variation
        finalColor *= mix(0.9, 1.1, detail);

        // Overall opacity with height and edge falloff
        float baseAlpha = max(coredensity, max(middensity, outerdensity));

        // Exponential falloff at top for wispy effect, and sharp falloff at edges
        float alpha = baseAlpha * smoothstep(1.0, 0.2, pow(heightGradientSmooth, 2.5)); // More aggressive top fade
        alpha *= smoothstep(1.0, 0.5, distFromCenter); // Sharper fade horizontally
        ...
        finalColor *= (0.7 + uPhaseIntensity * 0.6); // Slightly more base and stronger boost
